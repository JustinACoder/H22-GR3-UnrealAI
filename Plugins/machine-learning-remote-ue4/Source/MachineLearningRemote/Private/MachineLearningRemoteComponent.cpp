...
void UMachineLearningRemoteComponent::BeginPlay()
{
	...
	if (bAutoStartServer && !bServerIsRunning)
	{
		...
		if (bUseEmbeddedServer)
		{
			...
			// Changed the path of server.py to relative path from python version
			Process = FMLProcess::Create(EmbeddedServerPath + TEXT("/python.exe"), TEXT("../ml-remote-server/server.py -e"), true, false, false, 0, ServerScriptPath, true);
		}
		else
		{
